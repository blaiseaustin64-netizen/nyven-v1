// Cloudflare Pages Function — replaces api/build.ts (Vercel version)
// Same logic as before; only the handler signature and env access changed.

interface Env {
  OPENROUTER_API_KEY: string
  OPENROUTER_BUILDER_MODEL?: string
}
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const { description, websiteType, style, features } = body

    if (!description || typeof description !== 'string' || !description.trim()) {
      return jsonResponse(
        { success: false, error: 'Please describe the website you want NYVEN to build.' },
        400
      )
    }
    ...
    if (!apiKey) {
      ...
      return jsonResponse(
        { success: false, error: 'NYVEN Builder is temporarily unavailable. Please try again later.' },
        500
      )
    }

    const BUILDER_MODEL = env.OPENROUTER_BUILDER_MODEL || 'deepseek/deepseek-v4-flash-0731'
...
files must be complete polished client-side website code — not a basic template.`
    ...
    // schema is retained for validation expectations even though OpenRouter uses json_object
    ...
    if (
      ...
      return jsonResponse(
        { success: false, error: 'NYVEN could not produce a safe website for this request. Please try again.' },
        500
      )
    }
    ...
    return jsonResponse({ success: false, error: userMessage }, status)
  }
}
